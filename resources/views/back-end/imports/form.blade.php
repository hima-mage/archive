...
<div class="row">
    <div class="col-md-4">
        ...
        <div class="col-md-7"><label class="outborder">{{ $import_number !="" ? $import_number->id+1 : 1 }}</label></div>
    </div>
    <div class="col-md-4">
...
<div class="row">
    <div class="col-md-6">
        ...
        <div class="col-md-7"> 
            @php $input = "export_number"; @endphp
            <input type="number" name="{{ $input }}" id="{{ $input }}" value="{{ isset($row) ? $row->{$input} : old($input) }}"
                class="form-control">
        ...
        <div class="col-md-9">
            ...
            <select name="{{ $input }}" id="{{ $input }}" class="form-control" >
                ...
                @foreach ($categories as $category)
                    <option value="{{$category->id}}" {{ isset($row) && $category->id == $row->from_cat->id ? "selected" : "" }}>{{$category->name}}</option>
                @endforeach
            </select>
                <span class="{{ $input.'_error' }} invalid-feedback" role="alert"></span>
    ...
    <script>
        $(document).ready(function() {
            $(window).load(function() {
                setTimeout(function(){
                    $('#from_sub_category option[value="<?php echo $row->from_sub_category; ?>"]').prop('selected', true).change();
                }, 1000);
            });
        });
    </script>
...
<div class="row">
    <div class="col-md-6">
        ...
        <div class="col-md-9">
            ...
            <select  name="{{ $input }}" id="{{ $input }}" class="form-control" >
                <option value=""></option>
                <option value="sms" {{ isset($row) && $row->remind == "0" ? "selected" : "" }}>E-mail</option>
                <option value="email" {{ isset($row) && $row->remind == "1" ? "selected" : "" }}>SMS</option>
            </select>
                <span class="{{ $input.'_error' }} invalid-feedback" role="alert"></span>
    ...
    <div class="col-md-6">
        ...
        <div class="col-md-9">
            ...
            <input type="text"  name="{{ $input }}" id="{{ $input }}" value="{{ isset($row) ? $row->{$input} : old($input) }}" class="form-control" />
                <span class="{{ $input.'_error' }} invalid-feedback" role="alert"></span>
...
<div class="row">
    <div class="col-md-6">
        ...
        <div class="col-md-9">
            ...
            <input type="file"  name="{{ $input }}" id="{{ $input }}" value="{{ isset($row) ? $row->{$input} : old($input) }}" class="form-control" />
                <span class="{{ $input.'_error' }} invalid-feedback" role="alert"></span>
        ...
        $(document).ready(function() {
         ...
         $(window).load(function() {
            $.ajax({
                method: 'get',
                data:'',
                url: '/admin/sub_categories/catSub/'+$('#from_category').val(),
                success: function(response) {
                    $('#from_sub_category').html(response);
                }
            });
...
<script>
    $(document).ready(function() {
        //get from_departments of selected from_adminstration
        $('#from_category').change(function() {
            $.ajax({
                method: 'get',
                data:'',
